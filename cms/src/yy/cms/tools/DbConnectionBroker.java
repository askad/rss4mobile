...
public class DbConnectionBroker implements Runnable {
	...
	// comments "minConns" By yy,sinace never used it
	private int currConnections, connLast, maxConns, maxConnMSec, maxCheckoutSeconds, debugLevel;
	...
	public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns,
		...
		setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, logFileString, maxConnTime, logAppend,
				DEFAULTMAXCHECKOUTSECONDS, DEFAULTDEBUGLEVEL);
	}
	...
	private void setupBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns,
		...
		try {
			...
			try {
				log = new PrintWriter(new FileOutputStream("DCB_" + System.currentTimeMillis() + ".log", logAppend),
						true);
		...
		try {
			for (int i = 1; i < dbLoop; i++) {
				try {
					...
					if (debugLevel > 0) {
						log.println("--->Attempt (" + String.valueOf(i) + " of " + String.valueOf(dbLoop)
			...
			for (int i = 0; i < currConnections; i++) { // Do for each
				...
				try { // Test the connection with createStatement call
					...
					try {
						...
						if (debugLevel > 0) {
							log
									.println("Error!  Can't close connection!  Might have been closed already.  Trying to recycle anyway... ("
											+ e0 + ")");
						}
					}
	...
	public Connection getConnection() {
		...
		if (debugLevel > 2) {
			log.println("Handing out connection " + idOfConnection(conn) + " --> "
